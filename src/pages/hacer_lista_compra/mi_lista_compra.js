...
export default class MiListaCompra extends Component {
  ...
  //this.noRefresh=this.noRefresh.bind(this)
...
getProductsList() {
  ...
  axios
  .get("https://eficompra-back.onrender.com/productosdeSupermercados")
  .then(response => {
...
getOptions(){
  ...
  this.state.dataTotal.map(product=>{
      ...
      this.state.productsList.push({
        id:product.id,
        productName:`${product.name.toLowerCase().replace(product.trademark,"")} __ ${product.price} €`,
...
deleteDataBase(){
  ...
  this.state.data.map(i=>{
    return(
      axios
      .delete("https://eficompra-back.onrender.com/hacerCompra/i.idHacerCompra"),[i.idHacerCompra]
    )
    })
